...
void VehiclePlanner::RecalculateRoute() {
	...
	this->vis_->ShowPoint(vehicle_id_ * 1000 + 1, this->target_pos_, 0.05, VisualizationHandler::GREEN);
	...
	auto add_center_line_to_planned_states = [this](const auto &next_lanelet) {
		...
		for (size_t j = 0; j < center_line.size() - 1; j++) {
			VehicleState state = {this->time_, next_lanelet.id(), center_line[j].basicPoint2d(), {0.0, 0.0}, "VehiclePlanner"};
			this->AddState(state);
		}
	};
