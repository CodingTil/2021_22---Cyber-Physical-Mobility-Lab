...
void VehiclePlanner::TravelAlongPlannedStates(Time delta_t) {
  ...
  // Push current state on planned state buffer but ensure deletion at end of this functions
  long reached_states = 1;
  this->planned_states_.insert(this->planned_states_.begin(), this->current_state_);
  ...
  for (size_t i = 0; i < this->planned_states_.size() - 1; i++) {
    ...
    auto time_in_state = speed != 0 ? static_cast<Time>(distance / speed * 1e9) : 0ull;

    // Percent of distance covered between two states
    double factor = std::min(static_cast<double>(remaining_time) / static_cast<double>(time_in_state), 1.0);
    if (factor >= 1.0) {
      // 100% distance between states covered update buffer and iterate to next state
      remaining_time -= static_cast<Time>(static_cast<double>(time_in_state) * factor);
      reached_states++;
      continue;
    }

    // Not 100% distance between states covered update current position and break loop
  ...
  // Erase old states
  this->planned_states_.erase(this->planned_states_.begin(), this->planned_states_.begin() + reached_states);
}
...
void VehiclePlanner::UpdateCurrentState(Pos &position, long lanelet_id, const Vel &vel) {
  ...
  this->vis_->ShowPoint(this->vehicle_id_ * 1000 + 2, position, 0.05, VisualizationHandler::RED);
}

VehiclePlanner::VehiclePlanner(const Id vehicle_id, LaneGraphPtr map, VisualizationHandlerPtr vis) : vehicle_id_(
    vehicle_id), vis_(std::move(vis)), map_(std::move(map)), target_position_reached_(false) {}

void VehiclePlanner::InitialiseVehicleState(LaneletId lanelet_id, const Pos &pos, const Vel &vel, Time time) {
...
void VehiclePlanner::Update(Time delta_t) {
  ...
  if (target_position_reached_) return;
  ...
  if (this->planned_states_.empty()){
//    std::cout << "[VEHICLEPLANNER] Vehicle " << int(vehicle_id_) << " ready for pickup by system at: " << this->time_ << std::endl;
...
void VehiclePlanner::CalculateRouteToTargetPosition(const Pos &target_position) {
  if(this->planned_states_.empty()) this->AddState(current_state_);
  this->target_position_reached_ = false;

  // Print new target to console and show of visualisation is supported
//  std::cout.precision(2);
//  std::cout << "[VEHICLEPLANNER] Vehicle " << int(this->vehicle_id_);
//  std::cout << " on route to: [" << std::fixed << target_position.x() << " " << target_position.y() << "]" << std::endl;
  this->vis_->ShowPoint(vehicle_id_ * 1000 + 1, target_position, 0.05, VisualizationHandler::GREEN);

  // Get route to target from lanelet map
  auto last_planned_lanelet = this->map_->FindLaneletById(this->planned_states_.back().GetLaneletId());
  auto target_lanelet = this->map_->FindLaneletByPosition(target_position);
  auto route_to_target = this->map_->FindShortestRoute(last_planned_lanelet, target_lanelet);
  auto shortest_path = route_to_target->remainingShortestPath(last_planned_lanelet);

  auto add_center_line_to_planned_states = [this](const auto &next_lanelet) {
    // Do not add the same lanelet twice
    if (next_lanelet.id() == this->planned_states_.back().GetLaneletId()) return;
    auto center_line = next_lanelet.centerline();
    for (size_t j = 0; j < center_line.size() - 1; j++) {
      VehicleState
          state = {this->time_, next_lanelet.id(), center_line[j].basicPoint2d(), {0.0, 0.0}, "VehiclePlanner"};
      this->AddState(state);
    }
  };

  std::for_each(shortest_path.begin(), shortest_path.end(), add_center_line_to_planned_states);
}
